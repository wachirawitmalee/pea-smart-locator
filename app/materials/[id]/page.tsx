...
export default async function MaterialDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 font-sans py-8 px-4 sm:px-6 lg:px-8 pb-20">
      ...
      <Link  href="/" className="inline-flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-[#741F80] transition-colors group w-fit">
  <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" />
  ย้อนกลับไปหน้าค้นหาหลัก
</Link>
